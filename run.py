...
def got_solve_text(task, idx, to_print=True):
    prompt_sample = 'standard'
    n_generate_sample = 5
    n_neighborhood_sample = 5
    n_evaluate_sample = 5


    x = task.get_input(idx)
    init_solution = get_node_solution(task, x, '', 1, prompt_sample, stop=None)[0]
    warp_init_solution = task.plan_output_warp(init_solution)

    thought_map = {1:{'plan': warp_init_solution, 'sentence': x, 'neighborhood': None}}
    thought_map_iter = {} 
    write_passage = get_text_passage_samples(task, x, warp_init_solution, n_generate_sample, stop=None)
    ...
    for step in range(task.steps-1):
        for key, value in thought_map.copy().items():
            if value['neighborhood'] == None:
                neighborhood_problems = get_neighborhood_problem(task, x, '', n_neighborhood_sample)
                ...
                for nei_id in range(thought_count, thought_count + num_neigborhood):
                    ...
                    node_solution = get_node_solution(task, rephrase_x, '', 1, prompt_sample, stop=None)[0]#.strip('Plan:\n')
        ...
        write_passage = get_text_passage_samples(task, x, thought_map[1]['plan'], n_generate_sample, stop=None)#args.n_generate_sample
        concate_message = []
    ...
    return infos, thought_map_iter,final_message
    ...
    for run_iter in range(0, args.num_rum):
        ...
        for i in range(args.task_start_index, args.task_end_index): # args.task_end_index
            ...
            if args.naive_run:
                ...
                ys, info,_ = got_solve_text(task, i)
