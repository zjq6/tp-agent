...
class TextTask(Task):
    ...
    Output Example: 
    """
    def __init__(self, file='data_1_random_text.txt'):
        """
...
#注意这里还应该检查一下和原论文格式是否匹配，这里只是简单写入
def init_Task_by_string(problem_description):
    # 原论文中从file读取任务初始化Task，Task可能有多条
    # 这里简化，直接从字符串初始化Task
    #创建一个文件，写入problem_description
    with open('data/text/problem_description.txt', 'w') as f:
        f.write(problem_description)
    # 初始化Task
    task = TextTask("problem_description.txt")
    return task
